...
public class ObraRepository {
    ...
    public boolean save(Obra obra) throws IOException{
        ...
        for (Obra obrasNoDatabase : obras) {
            if(obra.getId() == obrasNoDatabase.getId()){
                return false;
            }
    ...
    public boolean update(Obra obra) throws IOException{
        ...
        for (Obra obrasNoDatabase : obras) {
            if( obra.getId()==obrasNoDatabase.getId()){
                obrasNoDatabase.setNome(obra.getNome());
                obrasNoDatabase.setAutor(obra.getAutor());
                obrasNoDatabase.setPreco(obra.getPreco());
                obrasNoDatabase.setJaFoiVendida(obra.jaFoiVendida());
