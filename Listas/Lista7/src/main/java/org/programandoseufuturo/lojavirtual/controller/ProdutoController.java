...
public class ProdutoController {
    ...
    public ResponseEntity<Produto> cadastrar(@RequestBody Produto produto){
        Produto produtoCadastrado = service.cadastraProduto(produto);
        return ResponseEntity
            .status(HttpStatus.CREATED.value()).body(produtoCadastrado);
    }

    @DeleteMapping("{id}")
    public ResponseEntity<Produto> deletarProduto(@PathVariable("id") int id){
        return ResponseEntity
            .status(HttpStatus.OK.value()).body(service.deletaProduto(id));
    }
}
